...
1. Install the required dependencies:
   ```bash
   pip install tensorflow opencv-python scikit-learn matplotlib streamlit imutils
   ```

2. Make sure the model file is in the `models` directory.
   - The app will look for `models/brain_tumor_model.keras` or `models/cnn-parameters-improvement-23-0.91.model`
...
2. Process the images for tumor detection
3. View the prediction results

### New UI highlights

- Sidebar controls for preprocessing (Auto-crop brain region)
- Multiple image uploads with batch predictions and a clean results grid
- Sample image picker (if `yes/` and `no/` folders exist at repo root)
- Professional theme configured via `.streamlit/config.toml`
- Subtle visual polish via `styles/app.css`

On Windows PowerShell, you can also run headless on a fixed port:
...
To train your own model using the provided dataset:

```bash
python brain_tumor_detection.py --train
```

## Making Predictions on Single Images

To make a prediction on a specific image:

```bash
python brain_tumor_detection.py --predict path_to_your_image.jpg
```
